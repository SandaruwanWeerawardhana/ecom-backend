...
/**
 * Reseller Pending Orders List Lookup Consumer
 * Handles pending orders list lookup requests from Reseller module via RabbitMQ
 * Maps to routing key: reseller.pending.orders.list.lookup.request
 *
 * Returns only orders with statuses: PENDING, PROCESSING, OUT_FOR_DELIVERY
 * Supports filtering by search (order number, customer name), date range, and pagination
 */
@Component
@RequiredArgsConstructor
@Slf4j
public class ResellerPendingOrdersListLookupConsumer {

    private final OrderRepository orderRepository;
    private final OrderShippingAddressRepository shippingAddressRepository;

    @RabbitListener(
            queues = "${app.rabbitmq.queue.reseller-pending-orders-list-lookup-request:reseller.pending.orders.list.lookup.request}",
            containerFactory = "orderRabbitListenerContainerFactory"
    )
    @Transactional("orderTransactionManager")
    public Map<String, Object> handleResellerPendingOrdersListLookup(Map<String, Object> request) {
        log.info("=== RESELLER PENDING ORDERS LIST LOOKUP CONSUMER TRIGGERED ===");
        log.info("Received pending orders list lookup request for reseller ID: {}", request.get("resellerId"));
        ...
        try {
            ...
            if (resellerId == null) {
                log.error("Invalid pending orders list lookup request - missing resellerId");
            ...
            log.info("Fetching pending orders for reseller ID: {} - search: {}, startDate: {}, endDate: {}, page: {}, size: {}",
                    resellerId, search, startDateStr, endDateStr, page, size);
            ...
            List<OrderEntity.OrderStatus> pendingStatuses = Arrays.asList(
            ...
            Page<OrderEntity> ordersPage = orderRepository.findResellerPendingOrdersWithFilters(
                    resellerId,
                    normalizedSearch,
                    pendingStatuses,
                    startDateTime,
            ...
            log.info("Pending orders fetched - Total: {}, Pages: {}, Current Page: {}",
            ...
            log.error("Error handling reseller pending orders list lookup request", e);
