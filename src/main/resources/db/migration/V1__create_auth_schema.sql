...
INSERT INTO user_permission (permission_code, permission_name, module, is_active) VALUES
('USER_READ', 'User Read', 'USER', TRUE),
('USER_WRITE', 'User Write', 'USER', TRUE),
('USER_DELETE', 'User Delete', 'USER', TRUE),
('PRODUCT_READ', 'Product Read', 'PRODUCT', TRUE),
('PRODUCT_WRITE', 'Product Write', 'PRODUCT', TRUE),
('PRODUCT_DELETE', 'Product Delete', 'PRODUCT', TRUE),
('ORDER_READ', 'Order Read', 'ORDER', TRUE),
('ORDER_WRITE', 'Order Write', 'ORDER', TRUE),
('ORDER_DELETE', 'Order Delete', 'ORDER', TRUE),
('ADMIN_ACCESS', 'Admin Access', 'ADMIN', TRUE)
ON DUPLICATE KEY UPDATE permission_name = permission_name;

-- Assign Permissions to ADMIN Role
INSERT INTO role_permission (role_id, permission_id, is_active)
SELECT
    (SELECT id FROM user_role WHERE role_code = 'ADMIN'),
    p.id,
    TRUE
FROM user_permission p
WHERE (SELECT id FROM user_role WHERE role_code = 'ADMIN') IS NOT NULL
